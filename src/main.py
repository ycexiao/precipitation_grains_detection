...
def CountGrain(file_name):
    ...
    bi_pic = my_separator.preprocess(picture, binary_threshold = 51, kernel_radius = 2, min_distance=2)
    ...
    for i in range(len(regions)):
        ...
        lines = SearchLines(skeleton)  # Separate lines that cross each other in a region
        ...
        for j in range(len(lines)):
            ...
            converted_lines = my_counter.HoughTransform(lines[j].memory, theta_sep=1/180*np.pi, rho_sep=1, rho_amplitude=100)

            if converted_lines is not None:
                converted_lines = [grain[0] for grain in converted_lines]  # reorganize the dimension of the array
                total_lines.extend(converted_lines)
            else:
                continue

        all_lines = my_counter.Approximate(total_lines, rho_error=3, theta_error=15/180*np.pi)  # merge the lines belonging the same lines
        valid_grains = my_counter.Vote(all_lines, voter_threshold = 5)  # filter the final lines that are too short
...
def aggreagtion(ks):
    ...
    kmeans = KMeans(n_clusters = 2, random_state = 0, n_init='auto').fit(ks.reshape([-1,1]))  # 由于只有两种方向，所以分为两类。考虑晶粒颜色的需要再进一步改动
    max_n = kmeans.labels_.max()

    statistics = []
    for i in range(max_n+1):
        statistics.append(sum(kmeans.labels_ == i))  # acquire the number of grains in separate classies

    centers = np.abs(kmeans.cluster_centers_.ravel())  # use the center to make sure all the classes output by different picture are the same classes( the same class share the same center, at least the same order of their center)
