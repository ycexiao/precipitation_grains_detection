...
class Separator(object):
    ...
    def preprocess(self, pic, binary_threshold = 51, kernel_radius = 2, min_distance=2):  # 这些参数是经过调试之后选择的，如效果不理想，首先考虑调整这些参数
        '''
        二值化、开运算+距离转换
        ...
        ret, bi_pic = cv2.threshold(pic, binary_threshold, 255, cv2.THRESH_BINARY)  # Binarize

        kernel = np.ones((kernel_radius, kernel_radius), np.uint8)  # the kernel applied to morphological calculation
        open_bi_pic = cv2.morphologyEx(bi_pic, cv2.MORPH_OPEN, kernel)

        rim_open_bi_pic = addrim(open_bi_pic)  # adding rims 
        distance = cv2.distanceTransform(rim_open_bi_pic, distanceType=cv2.DIST_L2, maskSize=5)  # 值为1的像素的值转换为到最近的值为0的像素的距离
        final_pic = (distance > min_distance) * 255  # 低于min_distance的像素会删除，即会删除晶粒的边缘像素。
    ...
    def Generate_regions(self,labels,pic ):
        ...
        get_boundary = lambda x: [x[:,0].min(),x[:,0].max(), x[:,1].min(),x[:,1].max()]
        ...
        for ind in range(1,n_objects+1):
            ...
            binary_region = (labels==ind)[min_x:max_x,min_y:max_y].astype('uint8')*255
            original_region = pic[min_x:max_x,min_y:max_y].astype('uint8')
            original_region = cv2.bitwise_and(original_region,original_region,mask=binary_region.astype('uint8'))
