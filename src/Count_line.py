...
class Counter:
    def __init__(
        self,
    ):  # rho is the distance between o and the line, it should be equal to the diagonal
        pass
    ...
    def HoughTransform(self, footsteps, theta_sep=1, rho_sep=1, rho_amplitude=100):
        ...
        lines = cv2.HoughLinesPointSet(
            ...
            5,
            5,
            min_rho=-rho_amplitude,
            max_rho=rho_amplitude,
            rho_step=rho_sep,
            min_theta=-np.pi / 2,
            max_theta=np.pi / 2,
            theta_step=theta_sep,
        )
        return lines
    ...
    def Approximate(self, lines, rho_error=3, theta_error=15 / 180 * np.pi):
        ...
        def weighted_distence(l1, l2, mode="p2p"):
            ...
            result = np.sqrt(
                ((x1 - x2) / rho_error) ** 2 + ((y1 - y2) / theta_error) ** 2
            )
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    ...
                    dist[i, j] = (
                        np.inf
                    )  # dist[i,i] = np.inf, so (dist[i,:]).argmin() can be actually the most similar line to lines[i]

        threshold = np.sqrt(
            2
        )  # above which is considered to be diffenrent lines.  # The error of rho and theta are weighted so it's ok to set threshold to sqrt(2)
        all_lines_ind = []
        ...
        while not (dist == np.inf).all():  # start classification
            ...
            """
            same_lines_ind = []
            i, j = np.argwhere(dist == dist.min())[
                0
            ]  # two lines with most similar parameters in current iteration  # i:row_number, j:column_number
            dist[:, j] = (
                np.inf
            )  # when update distance, we only set column_number to be np.inf
            ...
            while True:
                ...
                potential_deviation = [
                    weighted_distence(same_lines, lines[p], mode="s2p")
                    for p in potential_lines_ind
                ]

                if (
                    np.array(potential_deviation) >= threshold
                ).all() == True:  # if deviation are not tolerable
                    break
                else:
                    p = np.array(
                        potential_deviation
                    ).argmin()  # only add the most close line into 'same_lines_ind'
                    same_lines_ind.append(potential_lines_ind[p])
                    dist[:, potential_lines_ind[p]] = (
                        np.inf
                    )  # in case it will be found again by other LineBud
