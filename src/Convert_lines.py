...
def SearchLines(skeleton):
    ...
    def Look_around(linebud, skeleton):
        ...
        coordinates.remove(
            list(linebud.current_position)
        )  # current position is not recorded
        coordinates = np.array(coordinates)
        solid_points = np.argwhere(
            np.array([skeleton[i, j] for i, j in coordinates]) == 255
        )
    ...
    corner.extend(
        [
            solid_index[i]
            for i in [func(solid_index[:, 1]) for func in [np.argmin, np.argmax]]
        ]
    )  # left and right
    corner.extend(
        [
            solid_index[i]
            for i in [func(solid_index[:, 0]) for func in [np.argmin, np.argmax]]
        ]
    )  # top and bottom
    ...
    while total_buds != []:
        ...
        for j in range(len(total_buds)):
            current_position = total_buds[
                j
            ].current_position  # save the value in case it is changed by updating
            solid_coordinates = Look_around(
                total_buds[j], skeleton
            )  # solid pixels around the tip of current LineBud
            ...
            if len(solid_coordinates) == 0:  # there are no solid pixels around
                ...
                for k in range(len(solid_coordinates)):
                    if (
                        k == 0
                    ):  # searching along the first direction should not create new LineBud to bifurcate
        ...
        lines.extend(
            list(filter(lambda x: x.status == "close", total_buds))
        )  # lineBuds that stop grow
